<nav class="bg-white shadow-md" x-data="{ open: false, dropdownOpen: false }">
        ...
        <div class="flex justify-between items-center h-16">
            ...
            <div class="hidden sm:flex space-x-8 sm:-my-px sm:ms-10">
                ...
                @if(auth()->user() && auth()->user()->is_admin == 1)
                    <a href="{{ route('users.index') }}" class="text-gray-900 hover:text-blue-500">Usuarios</a>
                    <a href="{{ route('paquetes.index') }}" class="text-gray-900 hover:text-blue-500">Paquetes</a>
                <!-- Dropdown -->
                <div class="relative">
                    ...
                    <div x-show="dropdownOpen" @click.away="dropdownOpen = false" class="absolute right-0 mt-2 bg-white border border-gray-200 rounded-md shadow-lg w-48">
                        <a href="{{ route('profile.edit') }}" class="block px-4 py-2 text-sm text-blue-600 hover:bg-gray-100">Profile</a>
                        <form method="POST" action="{{ route('logout') }}">
                            @csrf
                            <button type="submit" class="block w-full text-left px-4 py-2 text-sm text-blue-600 hover:bg-gray-100">Log Out</button>
                        </form>
                    </div>
                </div>
                @endif
    ...
    <div x-show="open" @click.away="open = false" class="sm:hidden bg-white shadow-md">
        <div class="space-y-1">
            ...
            <!-- Verificar si el usuario es admin usando el campo is_admin -->
            @if(auth()->user() && auth()->user()->is_admin == 1)
                <a href="{{ route('users.index') }}" class="block text-gray-900 hover:text-blue-500 px-4 py-2">Usuarios</a>
                <a href="{{ route('paquetes.index') }}" class="block text-gray-900 hover:text-blue-500 px-4 py-2">Paquetes</a>
            <!-- Dropdown en móvil -->
            <div class="relative">
